...
class ProviderBase:
    ...
    def _get(
        ...
        raise AssertionError("unreachable: loop always returns or retries")  # pragma: no cover
...
class GitHubProvider(ProviderBase):
    def __init__(self, config: dict[str, Any]) -> None:
        super().__init__(
            ...
            timeout_key="github",
            token_key="github",
            rate_limit=float(config.get("github", {}).get("rate_limit", 1)),
            cache_ttl=60.0,
        )
    ...
    def fetch(self, repo_url: str) -> ProviderResponse:
        ...
        return ProviderResponse(
            self.name, ProviderStatus.SUCCESS, response.fetched_at, data=normalized
        )
...
class ScorecardProvider(ProviderBase):
    def __init__(self, config: dict[str, Any]) -> None:
        super().__init__(
            config, "scorecard", timeout_key="scorecard", rate_limit=1.0, cache_ttl=300.0
        )
...
class OSSConfig:
    ...
    def _load_config(self):
        ...
        try:
            ...
            self.config["scoring"].setdefault(
                "thresholds",
                {
                    "critical": 90,
                    "high": 80,
...
class OSSScorer:
    ...
    def create_oss_framework(self):
        ...
        data = [
            ...
            [
                ...
                "10%",
                "0-100",
                "Daily=100, Weekly=80, Monthly=60, Quarterly=30, Yearly=10",
            ],
            ...
            [
                ...
                "7%",
                "0-100",
                "Monthly=100, Quarterly=80, Biannual=50, Yearly=20",
            ],
            [
                "  d. Maintainer Response Rate",
                "5%",
                "0-100",
                ">90%=100, 70-90%=75, 50-70%=50, <50%=20",
            ],
            ...
            [
                ...
                "6%",
                "0-100",
                "Corp=100, Verified Individual=80, Anonymous=50, New Anonymous=20",
            ],
            ...
            [
                ...
                "9%",
                "0-100",
                "Multi-democratic=100, Low-risk=80, Partial high-risk=40, Majority high=10",
            ],
            ...
            [
                ...
                "10%",
                "0-100",
                "Policy+Bounty=100, Policy=80, Some docs=50, None=10",
            ],
            [
                "  d. Dependency Security",
                "5%",
                "0-100",
                "All updated=100, <3 minor=80, Some old=40, Vulnerable=0",
            ],
            ...
            [
                ...
                "4%",
                "0-100",
                "F500=100, Tech firms=80, Some commercial=50, Individual=20",
            ],
            [
                "  c. Community Engagement",
                "5%",
                "0-100",
                "Active forums=100, Regular issues=80, Some questions=50, Little interaction=20",
            ],
            [
                "TOTAL SCORE",
                "100%",
    ...
    def create_proprietary_additions(self):
        ...
        risk_heat_df = pd.DataFrame(
            risk_heat, columns=["Application Criticality", "Risk Level", "Approval Status", "Notes"]
        )
        ...
        geo_risk = [
            ...
            [
                ...
                "US/EU/5EYES",
                10,
                multipliers["verified_individual"],
                "Medium risk multiplier",
            ],
            [
                "Verified Individual",
                "Other Democracies",
                20,
                multipliers["verified_individual"] * 1.25,
                "Elevated risk multiplier",
            ],
            [
                "Verified Individual",
                "High-Risk Nations",
                75,
                multipliers["verified_individual"] * 2.5,
                "Very high risk multiplier",
            ],
            ...
            [
                ...
                "High-Risk Nations",
                100,
                multipliers["anonymous"] * 3.0,
                "Extreme risk - avoid",
            ],
        ]
        geo_risk_df = pd.DataFrame(
            geo_risk,
            columns=["Maintainer Type", "Location", "Risk Points", "Weight Multiplier", "Notes"],
        )

        return {"Risk_Heat_Mapping": risk_heat_df, "Geopolitical_Risk_Matrix": geo_risk_df}
        ...
        for i in range(0, len(cve_ids), 30):
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    logger.warning(
                        "EPSS API returned %s for batch [%s...]", resp.status_code, batch[0]
                    )
            except requests.RequestException as e:
    ...
    def _resolve_osv_ecosystem(self, ecosystem: str) -> str | None:
        ...
        for reg_name, reg_cfg in self.config.get("registries", {}).items():
            if not isinstance(reg_cfg, dict):
                continue
            if key in [lang.lower() for lang in reg_cfg.get("languages", [])]:
    ...
    def check_osv(self, package_name: str, ecosystem: str = "npm") -> dict:
        ...
        try:
            ...
            if resp.status_code != 200:
                logger.warning(
                    "OSV API returned %s for %s/%s", resp.status_code, osv_ecosystem, package_name
                )
    ...
    def _resolve_registry(self, ecosystem: str) -> str | None:
        ...
        for reg_name, reg_cfg in registries.items():
            if not isinstance(reg_cfg, dict):
                continue
            langs = [lang.lower() for lang in reg_cfg.get("languages", [])]
            if key in langs:
...
class OSSVisualizer:
    ...
    def create_dashboard(self, project_data: dict, show: bool = True):
        ...
        if "scores" in project_data:
            ...
            plt.subplot(1, 2, 1)
            plt.bar(
                scores.keys(), scores.values(), color=["#4CAF50", "#FFC107", "#F44336", "#2196F3"]
            )
    ...
    def interactive_selector(self):
        ...
        dropdown = Dropdown(
            options=self.scorer.proprietary["Risk_Heat_Mapping"][
        ...
        def update_requirements(criticality):
            with output:
                ...
                reqs = self.scorer.proprietary["Risk_Heat_Mapping"][
                    self.scorer.proprietary["Risk_Heat_Mapping"]["Application Criticality"]
                    == criticality
                ]
                display(
                    reqs.style.set_properties(
...
class OSSWorkflow:
    ...
    def evaluate_component(self, component_data: dict) -> dict:
        ...
        if "package_name" in component_data:
            ...
            ecosystem = component_data.get("ecosystem") or component_data.get("language", "")
            if ecosystem:
                dl_data = self.scorer.get_download_count(component_data["package_name"], ecosystem)
        ...
        if osv_status == "error":
            provider_warnings.append(
                "OSV provider unavailable — vulnerability/malicious-package " "check is incomplete"
            )
        ...
        # Apply weights
        weighted_scores = {
            k: v * (self.config["scoring"]["weights"][k] / 100) for k, v in scores.items()
        }
        ...
        if results.get("osv_data", {}).get("is_malicious") and self.config.get(
            "malicious_packages", {}
        ).get("auto_prohibit", True):
    ...
    def _calculate_security_score(self, results: dict) -> float:
        ...
        if osv_data.get("is_malicious") and self.config.get("malicious_packages", {}).get(
            "auto_prohibit", True
        ):
            return 0.0
...
def init_oss_analysis():
    ...
    try:
        ...
        display(
            Markdown(f"Loaded configuration with weights: {config.config['scoring']['weights']}")
        )
